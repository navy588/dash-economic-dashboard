...
# 개별 프레임 분리
(df_cpi, df_pcepi, df_wti, df_hh, df_rate, df_spread,
 df_unrate, df_civ, df_payems, df_hpi) = (
    dfs['CPI'], dfs['PCEPI'], dfs['WTI'], dfs['HH'],
    dfs['Rate'], dfs['Spread'], dfs['UNRATE'],
    dfs['CIVPART'], dfs['PAYEMS'], dfs['HPI']
)

# BEA 전처리, 슬라이더 설정, Dash 앱 초기화
series_order = [
...
DF_PCT     = (raw_pct.pipe(lambda df: df[df['LineDescription'].isin(series_order)])
                 .groupby(['Date','LineDescription'], as_index=False)['Value'].last()
                 .pipe(lambda grp: grp.assign(
                     LineDescription=pd.Categorical(
                         grp['LineDescription'], categories=series_order, ordered=True
                     ))
                 )
                 .pivot(index='Date', columns='LineDescription', values='Value')
                 .sort_index())
DF_CONTRIB = (raw_contrib.pipe(lambda df: df[df['LineDescription'].isin(series_order)])
                   .groupby(['Date','LineDescription'], as_index=False)['Value'].last()
                   .pipe(lambda grp: grp.assign(
                       LineDescription=pd.Categorical(
                           grp['LineDescription'], categories=series_order, ordered=True
...
def _infl_layout():
    return html.Div([
        ...
        dcc.RangeSlider(
            id='inf-slider',
            min=2000, max=years['cpi'][-1],
            value=[2000, years['cpi'][-1]],
...
def _comm_layout():
    return html.Div([
        ...
        dcc.RangeSlider(
            id='comm-slider',
            min=2000, max=years['comm'][-1],
            value=[2000, years['comm'][-1]],
...
def _lab_layout():
    return html.Div([
        ...
        dcc.RangeSlider(
            id='lab-slider',
            min=2000, max=years['lab'][-1],
            value=[2000, years['lab'][-1]],
...
def _house_layout():
    return html.Div([
        ...
        dcc.RangeSlider(
            id='house-slider',
            min=2000, max=years['hpi'][-1],
            value=[2000, years['hpi'][-1]],
...
def update_gdp(path, slider):
    ...
    fig = go.Figure(go.Scatter(x=dates, y=DF_PCT['Gross domestic product'].loc[dates], mode='lines', line_color=colors['Gross domestic product']))
    fig.update_layout(template='plotly_white', title=f"GDP — {dates[-1].year%100}.{dates[-1].month:02d}: {val:.2f}%", xaxis_tickformat="'%y.%m", yaxis_title='%', margin={'t':50,'b':40})
    ...
    cards.append(html.Div([dcc.Dropdown(id='contrib-dd', options=opts, value=len(DF_CONTRIB)-1, style={'width':'120px'}), dcc.Graph(id='contrib-graph')], className='graph-card'))
    for s in series_order[1:]:
        ser = DF_PCT[s].loc[dates]
        fig2 = go.Figure(go.Scatter(x=dates, y=ser, mode='lines', line_color=colors[s]))
        fig2.update_layout(template='plotly_white', title=f"{display_labels[s]} — {ser.index[-1].year%100}.{ser.index[-1].month:02d}: {ser.iloc[-1]:.2f}%", xaxis_tickformat="'%y.%m", yaxis_title='%', margin={'t':50,'b':40})
...
def update_contrib(path, idx):
    ...
    fig = go.Figure(go.Bar(x=[display_labels[s] for s in series_order], y=[vals[s] for s in series_order], marker_color=[colors[s] for s in series_order]))
    fig.update_layout(template='plotly_white', title=f"Contributions — {date.year} Q{(date.month-1)//3+1}", yaxis_title='%', margin={'t':50,'b':40})
    return fig

@app.callback([Output('cpi-graph','figure'), Output('pcepi-graph','figure')], Input('inf-slider','value'))
...
@app.callback([Output('wti-graph','figure'), Output('hh-graph','figure')], Input('comm-slider','value'))
...
@app.callback([Output('rate-graph','figure'), Output('spread-graph','figure')], Input('int-slider','value'))
...
@app.callback([Output('unrate-graph','figure'), Output('civpart-graph','figure'), Output('payems-graph','figure')], Input('lab-slider','value'))
...
@app.callback(Output('hpi-graph','figure'), Input('house-slider','value'))
...
@app.callback([Output('sidebar','style'), Output('page-content','style')],
              Input('btn_toggle','n_clicks'), State('sidebar','style'), State('page-content','style'))
